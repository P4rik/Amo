...
def plot_complexity():
    ...
    # Обчислення теоретичної складності O(n^2)
    try:
        array_lengths = [int(entry.get()) for entry in length_entries]
    except ValueError:
        messagebox.showerror(title="Помилка", message="Невірний ввід! Будь ласка, введіть цілі числа для довжини масивів.")
    ...
    theoretical_complexity = [n ** 2 for n in array_lengths]

    # Сортування за довжиною масивів
    array_lengths, theoretical_complexity = zip(*sorted(zip(array_lengths, theoretical_complexity)))

    # Побудова графіка за теоретичною складністю
    fig, ax = plt.subplots()
    ax.plot(array_lengths, theoretical_complexity, label='Теоретична складність O(n^2)', color='red')
    ax.set_title('Графік теоретичної складності O(n^2)')
    ax.set_xlabel('Розмір масиву')
    ax.set_ylabel('Час сортування (секунди)')
    ax.legend()
    ax.grid(True)
...
#Radbut
var = tk.IntVar()
radb1 = tk.Radiobutton(panel, text='Задати довжину матриць',variable=var, value=1, font=('Calibri', 25, 'bold'), bg='#977AF9',
